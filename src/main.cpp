...
const string strMessageMagic = "Shuttlecoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
            ...
            {
                    nSubsidy >>= (nHeight / 1000000);  // 10 coins halving every 1 mill blocks
            }
...
int64_t GetProofOfStakeReward(int nHeight, int64_t nCoinAge, int64_t nFees)
{
    int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);
           ...
           if(nBestHeight <= 100)
            ...
            {
            nSubsidy = nSubsidy * 1 ;  //1000% until block 30000
            }
			
			else if(nBestHeight <= 32000)
            {
            nSubsidy = nSubsidy * 2 ;  //2000% until block 32000
            }
			

            else if(nBestHeight <= 60000)
            {
            nSubsidy = nSubsidy * 1 ;  //1000% until block 60000
            }
			
			
			else if(nBestHeight <= 62000)
            {
            nSubsidy = nSubsidy * 2 ;  //2000% until block 62000
            }
            ...
            {
            nSubsidy = nSubsidy * 1 ;   //1000% until block 100000
            }
			
			else if(nBestHeight <= 2000000)
            {
            nSubsidy = nSubsidy * 0.05 ;  //50% until block 2000000
            }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Shuttlecoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Berlin show cancelled due to Bono voice issues";
        CTransaction txNew;
        txNew.nTime = 1535200116;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1535200116;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 1750610 : 1750610;
        ...
        assert(block.hashMerkleRoot == uint256("0x5c75935b321437e08c4d8b03a2be819be2fe4bfdead2a13c81d2edb78cb55755"));
...
unsigned char pchMessageStart[4] = { 0x74, 0xa1, 0x73, 0x62 };
